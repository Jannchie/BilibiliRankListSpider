...
from BilibiliRankListSpider.items import MysqlItem
import time
import json
import logging
...
class HighSpeedVideoSpider2(scrapy.spiders.Spider):
    ...
    custom_settings = {
        'ITEM_PIPELINES': {
            'BilibiliRankListSpider.pipelines.ApiMySqlPipeline': 300
        }
    }
    ...
    def parse(self, response):
        try:
            ...
            for each_key in keys:
                ...
                if subChannel != '':
                    channel = sub_channel_2_channel[subChannel]
                elif subChannel == '资讯':
                    if tid == 51:
                        channel == '番剧'
                    if tid == 170:
                    ...
                    channel = None
                item.append((aid,author,view,favorite,danmaku,coin,share,like,dislike,subChannel,title,datetime,channel))                
            o = MysqlItem()
            o['data'] = item
            yield o
